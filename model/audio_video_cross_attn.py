...
class LXRTXLayer(nn.Module):
    def __init__(self, config):
        ...
        self.speech_attention = BertCrossattLayer(config)
    ...
    def cross_att(self, lang_input, lang_attention_mask, visn_input, visn_attention_mask):
        # Cross Attention
        lang_att_output = self.speech_attention(lang_input, visn_input, ctx_att_mask=visn_attention_mask)
...
if __name__ == "__main__":
   ...
   # For language features: (batch_size, sequence_length, hidden_size)
   lang_feats = torch.randn(2, 20, 1024)
   # For language attention mask: (batch_size, sequence_length)
   lang_attention_mask = None

   # For vision features: (batch_size, num_boxes, hidden_size)
   visn_feats = torch.randn(2, 36, 1024)
   # For vision attention mask: (batch_size, num_boxes)
   ...
   # Forward pass
   lang_output, visn_output = lxrt_encoder(lang_feats, lang_attention_mask, visn_feats, visn_attention_mask)

   # Outputs
   print("Language Output Shape:", lang_output.shape)
   print("Vision Output Shape:", visn_output.shape)
