...
class VGG(nn.Module):
    def __init__(self, features, postprocess, buffer):
        super(VGG, self).__init__()
        self.postprocess = postprocess
        self.features = features
        window_dimension = {0.5: (512 * 4 * 6, 4096) ,0.0: (512 * 3 * 4, 2048), 0.25: (512 * 4 * 4, 3072)}

        self.embeddings = nn.Sequential(
            nn.Linear(window_dimension[buffer][0], 2048),
            nn.ReLU(True),
            nn.Linear(2048, 2048),
            nn.ReLU(True),
            nn.Linear(2048, 256),
        ...
        self.embeddings.apply(self.initialize_weights)
        if postprocess:
...
class Postprocessor(object):
    ...
    def postprocess(self, embeddings_batch):
        ...
        """
        # Ensure that _pca_matrix and _pca_means are on the same device as embeddings_batch
        self._pca_matrix = self._pca_matrix.to(embeddings_batch.device)
        self._pca_means = self._pca_means.to(embeddings_batch.device)
...
def _vgg(postprocess=False, buffer=0.0):
    return VGG(make_layers(), postprocess, buffer)


def vggish(postprocess=True, buffer=0.0):
    """
