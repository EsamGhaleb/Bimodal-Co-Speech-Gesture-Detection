...
class MFSLabeler(Labeler):
    def __init__(self, gcns_model, speech_model, labeler_name: str, recurrent_encoder: bool, training_seq: str, labelset_size: int, pad_id=-1, labels=['outside', 'begin', 'inside', 'end', 'bos'], classes=['outside', 'begin', 'inside', 'end'], max_seq_len=40, ngram_size=3, joints_embed_dim=256, audio_embed_dim=128, label_embed_dim=32, include_embeddings=False, hidden_size=128, **kwargs):
        ...
        self.lstm = nn.LSTM(audio_embed_dim, 512, batch_first=True)
    ...
    def forward(self, x, audio_data, y, eval=False, get_embeddings=False, get_predictions=False, e=None, lengths=None, keep_prob=0.9):
        ...
        if not get_predictions:
            ...
            N, S, audio_C = audio_data.shape
            audio_data = audio_data.view(N*S, audio_C)
            # inputs = self.feature_extractor(audio_data, return_tensors="pt", sampling_rate=16000)
            audio_embeddings = self.speech_embed(audio_data)
            audio_embeddings = audio_embeddings.last_hidden_state

            # apply mean pooling to the output of the last layer
            output, (hn, cn) = self.lstm(audio_embeddings)

            audio_embeddings = hn.squeeze(0)
            audio_embeddings = audio_embeddings.view(N, S, -1)



            # audio_embeddings = torch.mean(audio_embeddings, dim=1)
        
        s_audio = self.audio_logits_predictor(audio_embeddings)
