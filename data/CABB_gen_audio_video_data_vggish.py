...
def generate_data_sw(
    ...
    try:
        input_audio, sample_rate = librosa.load(audio_path.format(pair, speaker),  sr=16000)
    except:
        input_audio, sample_rate = librosa.load(audio_path.format(pair, pair, speaker),  sr=16000)
    ...
    for t in tqdm(range(start_ind, end_ind, time_offset*history), leave=False):
        ...
        for i in range(t, t + history*time_offset, time_offset):
            ...
            if audio_start_frame + corresponding_audio_window_size > len(input_audio):
                # apply zero padding
                audio_window = np.zeros((corresponding_audio_window_size, ))
                audio_window[:len(input_audio[audio_start_frame:])] = input_audio[audio_start_frame:]
            else:
                audio_window = input_audio[audio_start_frame: audio_start_frame + corresponding_audio_window_size]
            # transform the audio window 
            audio_window = audio_window.T
            # Extract features using VGGish
            vvgish_input = waveform_to_examples(audio_window, sample_rate, return_tensor=False, EXAMPLE_WINDOW_SECONDS=buffer_and_window_in_seconds[buffer], EXAMPLE_HOP_SECONDS=buffer_and_window_in_seconds[buffer])
            # Extract features using Wav2Vec2
            # For demonstration, just print the shape of last_hidden_state
            # apply mean pooling on the extracted features
            # convert the tensor to numpy array
            audio_features.append(vvgish_input.squeeze())
        ...
        current_ind += 1
    return all_speakers_fps, all_speakers_audio, sequences_lengths, all_sample_names, sequences_labels, [None]*len(sequences_lengths), current_ind
...
def gendata(
    ...
    all_speakers_audio = np.zeros((total_num_samples, history, int(buffer_and_window_in_seconds[buffer]*100), 64))
    ...
    for pair, speaker in tqdm(pair_speaker, leave=True):
        ...
        all_speakers_fps, all_speakers_audio, sequences_lengths, all_sample_names, sequences_labels, _, current_ind = ret_val
...
if __name__ == '__main__':
    ...
    parser.add_argument('--keypoints-path', type=str, help='The path to the numpy file')
    ...
    out_folder = project_directory + 'data/'
    history = 40
    new_gestures_info_path = project_directory+'data/sampled_gestures_info.pkl'
    ...
    out_folder = project_directory+'data/mm_data_vggish/'
